...
async function requestPairingCodeIfNeeded(socket) {
  ...
  await waitForPairingCodeReadiness(socket);

  const code = await requestPairingCodeWithRetry(socket, CONFIG.pairingPhoneNumber);
  console.log(`Codigo de pareamento para ${CONFIG.pairingPhoneNumber}: ${code}`);
  return true;
}

async function waitForPairingCodeReadiness(socket, timeoutMs = 15000) {
  await Promise.race([
    sleep(2500),
    new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        cleanup();
        reject(new Error("Tempo limite excedido aguardando o WhatsApp preparar o pareamento."));
      }, timeoutMs);
      ...
      const onUpdate = (update) => {
        ...
        if (update.connection === "close") {
          cleanup();
          reject(update.lastDisconnect?.error || new Error("Conexao encerrada antes do pareamento."));
        }
      };
...
async function requestPairingCodeWithRetry(socket, phoneNumber, maxAttempts = 5) {
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      const isRetryable = statusCode === DisconnectReason.connectionClosed || statusCode === 428;
      ...
      console.log(
        `Pareamento ainda nao ficou pronto (tentativa ${attempt}/${maxAttempts}). Tentando novamente...`
      );
      await sleep(2000);
    }
  }
